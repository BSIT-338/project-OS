 ...
 * kernel/fs/ramfs.c
 *
 * A simple in-memory filesystem backed by kernel heap allocations.
 * Supports files, directories, and symlinks entirely in RAM.
 * All data is lost when the system reboots or the filesystem is unmounted.
 * ============================================================================ */
...
// Heap functions are now provided by the memory manager
...
/* A single directory entry inside a ramfs directory */
...
/* Per-inode private data */
typedef struct {
    /* For regular files: heap buffer holding file contents */
    uint8_t  *data;
    uint64_t  capacity;     /* Allocated bytes */

    /* For directories: linked list of directory entries */
    ramfs_dirent_t *entries;

    /* For symlinks: target path string */
    char *link_target;
} ramfs_inode_data_t;
...
static int ramfs_create(inode_t *dir, const char *name, uint16_t mode,
                        inode_t **out) {
...
static int ramfs_unlink(inode_t *dir, const char *name) {
    ...
    while (*pp) {
        if (strncmp((*pp)->name, name, NAME_MAX) == 0) {
            ...
            *pp = victim->next;
            victim->inode->nlinks--;
            /* Free inode data if no more links and no refs beyond our own */
...
static int ramfs_rmdir(inode_t *dir, const char *name) {
    /* For simplicity, delegate to unlink (caller should check empty) */
    return ramfs_unlink(dir, name);
}

static int ramfs_rename(inode_t *old_dir, const char *old_name,
    ...
    for (ramfs_dirent_t *de = opriv->entries; de; de = de->next) {
        if (strncmp(de->name, old_name, NAME_MAX) == 0) {
            ...
            if (new_dir != old_dir) {
                /* Move entry to new_dir's list — simplified (no atomic move) */
                (void)new_dir;
            }
            return 0;
        }
...
static void ramfs_sync(inode_t *node) { (void)node; /* No-op: already in RAM */ }
...
static int ramfs_file_write(file_t *file, const void *buf, size_t count) {
    ...
    if (new_end > priv->capacity) {
        ...
        if (priv->data) {
            memcpy(nb, priv->data, (size_t)priv->capacity);
            kfree(priv->data);
        }
        priv->data     = nb;
...
/* ============================================================================
 * VFS interface wrappers (Path-based operations)
 * ============================================================================ */
static int ramfs_fs_open(mountpoint_t *mnt, const char *path, int flags, 
                        uint16_t mode, file_t *file) {
    inode_t *node = ramfs_get_inode_by_path(mnt->root, path);
    
    if (!node && (flags & O_CREAT)) {
        /* Need to create the file. Find parent directory. */
        ...
        if (ramfs_create(dir, bname, S_IFREG | (mode & 0777), &node) != 0)
            return -1;
    }
...
static inode_t *ramfs_mount(fs_type_t *type, const char *source,
                            uint32_t flags) {
...
/* Public: create a standalone root inode (used before VFS is fully up) */
